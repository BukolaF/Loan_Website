...
const Navbar = () => {
  ...
  return (
    <div className='flex justify-between items-center h-24 max-w-[1240px] mx-auto px-4 py-4 text-blue-800'>
        <h1 className=' flex gap-2.5 w-full text-3xl font-bold text-blue-700'>Secured<span className='text-blue-500'>Loans</span> <GiTakeMyMoney className='h-8 w-8' size={20}/></h1>
        <ul className='hidden md:flex'>
        {navItems.map(({path,link}) => <li className='p-8'>
        <NavLink to={path}>{link}</NavLink> </li>)}
</ul>
        <button onClick={handleNav} className='block md:hidden self-center nav-link'>
          ...
          <h1 className='flex gap-2.5 w-full text-3xl font-bold text-blue-700 m-4 pt-6'>SecuredLoans <GiTakeMyMoney className='h-8 w-8' size={20} />     
          <IoMdCloseCircle onClick={()=>{handleNav(showNav)}} className='flex justify-center' />
                </h1>
